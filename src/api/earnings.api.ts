...
export interface Information {
  total_device: number;
  total_device_active: number;
  total_device_inactive: number;
}
...
export const getInformation = (id: number): Promise<Information> => {
  return new Promise((res) => {
    setTimeout(() => {
      res({
        total_device: 22,
        total_device_active: 20,
        total_device_inactive: 2,
      });
    }, 0);
  });
};
